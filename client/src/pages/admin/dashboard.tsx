...
export default function AdminDashboard() {
  ...
  const dashboardStats = [
    { label: 'Celkové objednávky', value: '1,234', change: '+12%', color: 'bg-blue-500', icon: '📦' },
    { label: 'Tržby (EUR)', value: '45,320', change: '+8%', color: 'bg-green-500', icon: '💰' },
    { label: 'Noví zákazníci', value: '89', change: '+5%', color: 'bg-purple-500', icon: '👥' },
    { label: 'Čakajúce platby', value: '12', change: '-2%', color: 'bg-orange-500', icon: '⏳' },
  ];
  ...
  const navItems = [
    ...
    { href: '/admin/customers', icon: '👥', label: 'Zákazníci' },
    { href: '/admin/discounts', icon: '🏷️', label: 'Zľavy' },
              ...
              {dashboardStats.map((stat, idx) => (
                <div key={idx} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
                  ...
                  <p className={`text-sm font-medium mt-4 ${stat.change.startsWith('+') ? 'text-green-600' : 'text-red-600'}`}>
                    {stat.change} oproti minulému mesiacu
                  </p>
                </div>
              ))}
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100">
              <div className="p-6 border-b border-gray-200">
                <div className="flex justify-between items-center">
                  <h2 className="text-lg font-bold text-gray-900">Posledné objednávky</h2>
